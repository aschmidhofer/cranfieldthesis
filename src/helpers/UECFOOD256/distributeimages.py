...
def main(_):
  ...
  _, _, train_filenames = get_random_cached_bottlenecks(
             None, image_lists, -1, 'training',
             FLAGS.bottleneck_dir, FLAGS.image_dir, None,
             None)
             
  _, _, validation_filenames = (
            get_random_cached_bottlenecks(
                None, image_lists, -1, 'validation',
                FLAGS.bottleneck_dir, FLAGS.image_dir, None,
                None))
                
  _, _, test_filenames = (
        get_random_cached_bottlenecks(None, image_lists, -1,
  ...
  #create sub dirs
  basedir = FLAGS.output_dir
  traindir = os.path.join(basedir,'training')
  validir = os.path.join(basedir,'validation')
  testdir = os.path.join(basedir,'testing')
  print("creating %s" %traindir)
  print("creating %s" %validir)
  print("creating %s" %testdir)
  os.makedirs(traindir)
  os.makedirs(validir)
  os.makedirs(testdir)
  
  # copy files to directories
  def copyall(filenames, destination_directory):
    for fpath in filenames:
      name = os.path.basename(fpath)
      dirname = os.path.basename(os.path.dirname(fpath))
      destdir = os.path.join(destination_directory, dirname)
      if not os.path.exists(destdir):
        os.mkdir(destdir)
        print("created %s" %destdir)
      destination = os.path.join(destdir,name)
      shutil.copyfile(fpath, destination)
  
  copyall(train_filenames, traindir)
  copyall(validation_filenames, validir)
  copyall(test_filenames, testdir)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=str,
      default='.',
      help='Path to folders where to store images.'
  )
  parser.add_argument(
      '--output_graph',
      type=str,
